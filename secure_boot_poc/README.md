...
4. Stage 0 writes the verified U-Boot bytes into `runtime/stage0/` as the authenticated handoff.
5. U-Boot verifies a signed Linux payload manifest, checks the hashes of the kernel payloads listed in that manifest, and writes the verified payload bytes into `runtime/uboot/`.
...
- `runtime/`: verified handoff copies produced by each boot stage
